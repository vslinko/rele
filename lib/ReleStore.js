...
export default class ReleStore extends Store {
  ...
  handleStartOptimisticRequest({requestId, addToLinkage, removeFromLinkage, add, merge, remove}) {
    this.requests.push(requestId);

    if (addToLinkage.length > 0) this.linkAdditions.push([requestId, addToLinkage]);
    if (removeFromLinkage.length > 0) this.linkDeletions.push([requestId, removeFromLinkage]);
  ...
  fulfill(query) {
    ...
    const recursiveClass = (cls, fields, resource) => {
      ...
      cls.block.includes.forEach(include => {
        ...
        if (Array.isArray(linkage)) {
          const additions = linkAdditions
            .filter(([type, id, name]) => type === result.type && id === result.id && name === include.name)
          ...
          result[include.name] = linkage
            .filter(linkage => {
              return !deletions.some(d => {
                return get(d, 'id') === linkage.id && get(d, 'type') === linkage.type;
              })
            })
            .map(readLinkage)
            .concat(additions);
      ...
      return this.rele.wrapper(result);
    };

    const result = recursiveCall(query);
    return fulfilled ? result : null;
  }
}
